...
export class Layer {
  ...
  _addIcons(context, map, maxDimension = globalMaxIconDimension) {
  ...
  _createElementHTML(context) {
    ...
    if (context.image !== undefined) {
      const image = JSON.parse(context.image);
  ...
  _setSidebar(content) {
    console.log('_setSidebar() ', 'this: ', this);
    const sidebarContent = document.getElementById('content');
    sidebarContent.innerHTML = `<div class="contentCard.${this.id}">${content}</div>`
  }
  ...
  _GeoJSONevents() {} // this goes into the subclasses. PointLayers should not have these
  ...
  show() {
    this._setSidebar(this._createElementHTML(this))
    console.log('show()');
  }
...
export class NetworkLayer extends SidebarLayer {
  ...
  add(map) {
    ...
    map.on('click', this.name, function (element) {
      ...
      let html = createElementHTML(element);
      const sidebarContent = document.getElementById('content');
      sidebarContent.innerHTML = '<button class="mapboxgl-sidebar-close-button" type="button" aria-label="Close popup">×</button><div class="contentCard">' + html + '</div>';
