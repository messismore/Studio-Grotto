...
export class Layer {
  ...
  _setMouseClick(map, action) {
    map.on('click', this.name, action(element))
  }
  ...
  _setSidebar(content) {
    ...
    sidebarContent.innerHTML = `<div class="contentCard ${this.id}"><button class="mapboxgl-sidebar-close-button" type="button">×</button>${content}</div>`
...
export class CardLayer extends Layer {
  ...
  _drawGeoJSONSymbolLayer(map) {
    ...
    // also, make add hover effects, when clicked, open a popup
  }
